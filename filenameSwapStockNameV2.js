...
const gorselKlasoru = "C:/Users/hamzaa/Desktop/Timon"
const excelDosyasi = "C:/Users/hamzaa/Desktop/Timon.xls"
...
files.forEach( file => {
  jsonData.forEach(data => {
    const barkod = data["Stok Ad"]; //ONEMLI ALAN
    const filenameSplitDot = file.split(".");
    const imageName = filenameSplitDot[0].trim()
    const imageNameNew = imageName.split("_")[0]
      if(barkod == undefined) {
        return;
      }
        if (imageName.includes('-') == false){
            if (barkod.includes(imageNameNew)) {
